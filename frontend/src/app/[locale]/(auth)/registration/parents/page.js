...
const ParentsRegistration = () => {
  const t = useTranslations("RegistrationParents");
  const register = useAuthStore((state) => state.registerParents);
  const googleCompleteSetup = useAuthStore(
    (state) => state.googleCompleteSetup,
  );
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        const data = JSON.parse(raw);
        if (data.role === 2) localStorage.removeItem("google-setup-data");
      } catch {
        /* noop */
      }
    }
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        router.push("/registration/verify-otp?type=verify&userType=parents");
      } catch (error) {
  ...
  return (
    <div className="min-h-screen to-blue-50 flex items-center justify-center p-4">
        ...
        <div className="card-body gap-4">
          <form onSubmit={handleRegisterSubmit}>
            ...
            <div className="flex gap-3">
              ...
              <p className="text-lg font-medium text-black">{t("header")}</p>
            </div>
            <p className="text-sm text-gray-600 my-2">{t("subtitle")}</p>

            {/* Google setup banner */}
            {googleSetupData && (
              <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-lg p-3 my-2">
                {googleSetupData.avatar_url && (
                  <img
                    src={googleSetupData.avatar_url}
                    alt="Google profile"
                ...
                <p className="text-sm text-blue-900">
                  ...
                  <span className="font-medium">{googleSetupData.email}</span>.
            ...
            {googleSetupData ? (
              ...
              <div className="w-full mx-auto my-3">
                <div className="tabs tabs-lifted rounded-lg">
                  ...
                  <label
                    htmlFor="tab-email"
                    className="tab flex items-center gap-2 w-1/2
                      bg-gray-100 text-black
                      peer-checked/email:bg-[#0052b4]
                      peer-checked/email:text-white
                  ...
                  <div className="tab-content bg-white rounded-md mt-2 hidden peer-checked/email:block">
                  ...
                  <label
                    ...
                    className="tab flex items-center gap-2 w-1/2
                      bg-gray-100 text-black
                      peer-checked/phone:bg-[#0052b4]
                      peer-checked/phone:text-white
                      rounded-md transition"
                  >
                    <Phone size={16} />
                    {t("phoneTab")}
                  ...
                  <div className="tab-content bg-white rounded-md mt-2 hidden peer-checked/phone:block">
                    <div className="form-control gap-1">
                      <span className="label-text font-medium text-black">
                        {t("phoneNumberLabel")}
                      </span>
            ...
            {/* Info alert */}
            <div className="alert alert-info bg-blue-50 border-[1px] border-blue-200 shadow-none my-3">
            ...
            <div className="grid grid-cols-4 my-2">
              <Link href="/registration">
                <button className="btn btn-ghost w-32 border-[1px] border-gray-200 rounded-lg shadow-none">
                  {t("backButton")}
                </button>
              </Link>
              <button
            ...
            {!googleSetupData && (
              <>
                ...
                {googleAccountExists && (
                  <div className="alert bg-yellow-50 border border-yellow-300 text-yellow-900 text-sm rounded-lg p-3 mb-2">
